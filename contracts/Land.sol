...
contract Land {
    ...
    function addLand(uint _area, string memory _city,string memory _state, uint landPrice, uint _propertyPID,uint _surveyNum,string memory _ipfsHash, string memory _document) public {
        ...
        landsCount++;
        lands[landsCount] = LandDetails(landsCount, _area, _city, _state, landPrice,_propertyPID, _surveyNum, _ipfsHash, _document);
        LandOwner[landsCount] = msg.sender;
        // emit AddingLand(landsCount);
    }

    //registration of seller
    function registerSeller(string memory _name, uint _age, string memory _ghanaCardNumber, string memory _tinNumber, string memory _landsOwned, string memory _document) public {
        ...
        sellersCount++;
        SellerMapping[msg.sender] = Seller(msg.sender, _name, _age, _ghanaCardNumber,_tinNumber, _landsOwned, _document);
        sellers.push(msg.sender);
        emit Registration(msg.sender);
    }

    function updateSeller(string memory _name, uint _age, string memory _ghanaCardNumber, string memory _tinNumber, string memory _landsOwned) public {
        ...
        SellerMapping[msg.sender].ghanaCardNumber = _ghanaCardNumber;
        SellerMapping[msg.sender].tinNumber = _tinNumber;
    ...
    function getSellerDetails(address i) public view returns (string memory, uint, string memory, string memory, string memory, string memory) {
        return (SellerMapping[i].name, SellerMapping[i].age, SellerMapping[i].ghanaCardNumber, SellerMapping[i].tinNumber, SellerMapping[i].landsOwned, SellerMapping[i].document);
    }

    function registerBuyer(string memory _name, uint _age, string memory _city, string memory _ghanaCardNumber, string memory _tinNumber, string memory _document, string memory _email) public {
        ...
        buyersCount++;
        BuyerMapping[msg.sender] = Buyer(msg.sender, _name, _age, _city, _ghanaCardNumber, _tinNumber, _document, _email);
        buyers.push(msg.sender);

        emit Registration(msg.sender);
    }

    function updateBuyer(string memory _name,uint _age, string memory _city,string memory _ghanaCardNumber, string memory _email, string memory _tinNumber) public {
        ...
        BuyerMapping[msg.sender].ghanaCardNumber = _ghanaCardNumber;
        BuyerMapping[msg.sender].email = _email;
        BuyerMapping[msg.sender].tinNumber = _tinNumber;
    ...
    function getBuyerDetails(address i) public view returns ( string memory,string memory, string memory, string memory, string memory, uint, string memory) {
        return (BuyerMapping[i].name,BuyerMapping[i].city , BuyerMapping[i].tinNumber, BuyerMapping[i].document, BuyerMapping[i].email, BuyerMapping[i].age, BuyerMapping[i].ghanaCardNumber);
    }
