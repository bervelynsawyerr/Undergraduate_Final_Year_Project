...
class ApproveRequest extends Component {
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      const requestsCount = await this.state.LandInstance.methods.getRequestsCount().call();
      ...
      for (let i = 1; i < requestsCount + 1; i++) {
        const request = await this.state.LandInstance.methods.getRequestDetails(i).call();
        console.log(request);
        // console.log(request[0].toLowerCase());
        // console.log(currentAddress);
        const isPaid = await this.state.LandInstance.methods.isPaid(request[2]).call();
        console.log(isPaid);
      ...
      <div className="content">
        <DrizzleProvider options={drizzleOptions}>
          <LoadingContainer>
            <Card>
              <CardHeader>
                <CardTitle tag="h4">Lands Transfer Request Info</CardTitle>
              </CardHeader>
              <CardBody>
