...
class BuyerInfo extends Component {
    ...
    NotifyBuyer = (item, email, name) => async () => {
        // alert('Before mail');
        sendMail(email, name);
        // alert('After mail');
    ...
    componentDidMount = async () => {
        ...
        try {
            ...
            const buyersCount = await this.state.LandInstance.methods.getBuyersCount().call();
            ...
            const verified = await this.state.LandInstance.methods.isLandInspector(currentAddress).call();
            //console.log(verified);
            this.setState({ verified: verified });

            for (let i = 0; i < buyersCount; i++) {
                const buyer = await this.state.LandInstance.methods.getBuyerDetails(buyersMap[i]).call();
                const buyer_verify = await this.state.LandInstance.methods.isVerified(buyersMap[i]).call();
                console.log(buyer_verify);
                buyer.verified = buyer_verify;

                const not_verify = await this.state.LandInstance.methods.isRejected(buyersMap[i]).call();
                ...
                buyerTable.push(<tr><td>{i + 1}</td><td>{buyersMap[i]}</td><td>{buyer[0]}</td><td>{buyer[5]}</td><td>{buyer[4]}</td><td>{buyer[1]}</td><td>{buyer[6]}</td><td>{buyer[2]}</td><td><a href={`https://ipfs.io/ipfs/${buyer[3]}`} target="_blank">Click Here</a></td>
                    ...
                    <td>
                        <Button onClick={this.NotifyBuyer(buyersMap[i], buyer[4], buyer[0])} disabled={buyer_verify || not_verify} className="btn btn-danger">
                            Reject
                        </Button>
                    </td>
                </tr>)
